...
-- Create the updated function with previous values in audit log
...
BEGIN
    ...
    LOOP
        ...
        IF v_field = ANY(v_allowed_fields) THEN
            ...
            -- Store both new and previous values
            v_update_fields := v_update_fields || jsonb_build_object(v_field, v_value);
            v_previous_values := v_previous_values || jsonb_build_object(v_field, v_old_data->v_field);
        ELSE
    ...
    -- Prepare audit details with both previous and new values
...
COMMENT ON FUNCTION public.update_user_profile_v2 IS 'Securely updates a user profile, including role and password changes if provided. Only admins can update other users profiles. Includes previous values in audit log.';
